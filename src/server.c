...
void *pre_login_srv(void *param)
{
    ...
    while (1)
    {
        ...
        case REGISTER_REQ:
            ...
            break;
        case ACTIVATE_REQ:
            handle_activate(conn_socket, acc_list);
            break;
        case LOGIN_REQ:
    ...
    if (result == LOGIN_SUCC)
    {
        ...
        for (int i = 0; i < MAX_USER; i++)
        {
            ...
            if (user[i].socket <= 0)
            {
                ...
                printf("%d %s %d\n", i, user[i].username, user[i].socket);
                break;
            }
        }
    }
...
void sv_user_use(int conn_socket)
{
    ...
    while (login)
    {
        ...
        case CHANGE_PASSWORD:
            sv_change_password(conn_socket, &pkg);
            break;

        case SEARCH_USERS:
            sv_search_user(conn_socket, &pkg);
            break;
...
void sv_active_user(int conn_socket, Package *pkg)
{
    ...
    for (int i = 0; i < MAX_USER; i++)
    {
        if (user[i].socket > 0)
        {
            // printf("%d %s %d", i, user[i].username, user[i].socket);
...
void sv_search_user(int conn_socket, Package *pkg)
{
    pkg->ctrl_signal = INCORRECT_ACC;
    for (int i = 0; i < MAX_USER; i++)
    {
        if (user[i].socket == conn_socket)
        {
            Account *acc = find_account(acc_list, user[i].username);
            if (acc == NULL)
                break;
            pkg->ctrl_signal = acc->status;
            break;
        }
    }
    send(conn_socket, pkg, sizeof(*pkg), 0);
...
void sv_change_password(int conn_socket, Package *pkg)
{
    char password[PASSWORD_SIZE];
    ...
    while (1)
    {
        printf("Receiving password from client\n");
        int res = recv(conn_socket, pkg, sizeof(*pkg), 0);
        printf("%d\n", res);
        printf("%d\n", pkg->ctrl_signal);
